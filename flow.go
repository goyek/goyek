...
func (f *Flow) runMain(args []string, opts ...Option) int {
	out := f.Output()
	// ensure that the output is synchronized
	// it is restored in the deferred function
	// so that the next run of Main or Execute is not affected
	oldOutput := f.output
	f.output = internal.SyncWriter(out)
	defer func() {
		f.output = oldOutput
	}()
	...
	go func() {
		...
		case <-sigChan:
			...
			osExit(exitCodeFail)
		case <-handlerDone:
			// finish consuming signals
			for {
				select {
				case <-sigChan:
					fmt.Fprintln(f.output, "additional interrupt ignored during cleanup")
				case <-handlerDone:
					return
				}
			}
